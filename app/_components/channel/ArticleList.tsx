...
export default function ArticleList({ articles, showActions = false, showCreateButton = false, onDelete }: ArticleListProps) {
  ...
  const handleEditClick = (article: Article) => {
    ...
    setEditFormData({ title: article.title, content: article.content });
    setSelectedImages([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isCreating) {
        ...
        console.log('Creating new article:', editFormData, selectedImages);
        alert('게시글이 작성되었습니다.');
      } else if (editingArticle) {
        // Update article logic here
        console.log('Updating article:', editingArticle.id, editFormData, selectedImages);
        alert('게시글이 수정되었습니다.');
      }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Image Zoom Modal for Create/Edit */}
      {zoomedImageIndex !== null && (isCreating || editingArticle) && (
